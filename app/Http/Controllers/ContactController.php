...
{
    public function sendMessageFromPage(Request $request)
    {
        ...
        try {
            ...
            // Send email
            Mail::send('emails.contact-page', $data, function ($mail) use ($data, $mailFromAddress, $mailFromName, $mailToAddress) {
                $mail->from($mailFromAddress, $mailFromName)
                    ->to($mailToAddress)
                    ->replyTo($data['email'], $data['name'])
                    ->subject('Contact Page: ' . $data['subject']);
            });

            return redirect()->back()->with('success', 'Thank you for contacting us! We will get back to you soon.');

        } catch (\Exception $e) {
            \Log::error('Contact page form submission error', [
                'error' => $e->getMessage(),
    ...
    public function sendMessage(Request $request)
    {
        ...
        try {
            ...
            $mailFromAddress = config('mail.from.address') ?: 'v4peacecounselling@gmail.com';
            $mailFromName = config('mail.from.name') ?: 'V4Peace';
            $mailToAddress = env('MAIL_TO_ADDRESS', 'v4peacecounselling@gmail.com');
